...
(
  ...
  output reg          muldiv_mux_sel_X3hl, // muldiv res from x3 instead of x
  ...
  // Bypassign Signal
  output  [2:0]   rs1_byp_sel_Dhl, // RF, X, M, W, X2, X3
  output  [2:0]   rs2_byp_sel_Dhl,
  ...
  // integradt
  assign muldivreq_val = muldivreq_val_Dhl && inst_val_Dhl && !stall_md_Dhl;
  ...
  wire stall_md_Dhl = (muldivreq_val_Dhl && inst_val_Dhl && (( is_md_Xhl && inst_val_Xhl && stall_Xhl) ));
  wire dep_X  = (rs1_en_Dhl && (rs1_addr_Dhl == rf_waddr_Xhl) && (rf_waddr_Xhl != 5'd0)) ||
                                    (rs2_en_Dhl && (rs2_addr_Dhl == rf_waddr_Xhl) && (rf_waddr_Xhl != 5'd0));
                    

  wire dep_M = (rs1_en_Dhl && (rs1_addr_Dhl == rf_waddr_Mhl) && (rf_waddr_Mhl != 5'd0)) ||
                                    (rs2_en_Dhl && (rs2_addr_Dhl == rf_waddr_Mhl) && (rf_waddr_Mhl != 5'd0));

  wire dep_X2 = (rs1_en_Dhl && (rs1_addr_Dhl == rf_waddr_X2hl) && (rf_waddr_X2hl != 5'd0)) ||
                                    (rs2_en_Dhl && (rs2_addr_Dhl == rf_waddr_X2hl) && (rf_waddr_X2hl != 5'd0));

                 
  wire stall_md_hzd_Dhl =  inst_val_Dhl && 
                                  ( (inst_val_Xhl && is_md_Xhl && dep_X) || 
                                  ( inst_val_Mhl && is_md_Mhl && dep_M) || 
                                  ( inst_val_X2hl && is_md_X2hl && dep_X2 ));
  
  //////////////////////////////////////////////////////////////
  /////////is_load pipeline ////////
  assign is_md_Dhl = (cs[`RISCV_INST_MSG_MULDIV_EN]);
  assign is_load_Dhl = (cs[`RISCV_INST_MSG_MEM_REQ] == 2'd1); // ld
  // is_load ?
  always @ (posedge clk) begin
    if (reset) begin
      is_md_Xhl   <= 1'b0;
...
///////////////////////////////////////////////////////////////////////
  wire can_bypass_X = inst_val_Xhl && rf_wen_Xhl && !is_load_Xhl && !is_md_Xhl;
  wire can_bypass_M = inst_val_Mhl && rf_wen_Mhl && !is_md_Mhl;
  wire can_bypass_X2 = inst_val_X2hl && rf_wen_X2hl && !is_md_X2hl;

  ///////////////////////////////////////////////////////////////////////////////              
  wire rs1_X_byp_Dhl = rs1_en_Dhl && inst_val_Dhl && can_bypass_X &&
                       (rs1_addr_Dhl == rf_waddr_Xhl) && (rf_waddr_Xhl != 5'd0);
  wire rs1_M_byp_Dhl = rs1_en_Dhl && inst_val_Dhl && can_bypass_M && 
                       (rs1_addr_Dhl == rf_waddr_Mhl) && (rf_waddr_Mhl != 5'd0);
  wire rs1_X2_byp_Dhl = inst_val_Dhl && inst_val_X2hl && rs1_en_Dhl &&
                      rf_wen_X2hl && !is_md_X2hl && (rs1_addr_Dhl == rf_waddr_X2hl) &&
                      (rf_waddr_X2hl != 5'd0);
  wire rs1_X3_byp_Dhl = inst_val_Dhl && inst_val_X3hl && rs1_en_Dhl && rf_wen_X3hl && (rs1_addr_Dhl == rf_waddr_X3hl) &&
                      (rf_waddr_X3hl != 5'd0);
  wire rs1_W_byp_Dhl = inst_val_Dhl && inst_val_Whl && rs1_en_Dhl && rf_wen_Whl && (rs1_addr_Dhl == rf_waddr_Whl) &&
                      (rf_waddr_Whl != 5'd0);
  // rs2
  wire rs2_X_byp_Dhl = rs2_en_Dhl && inst_val_Dhl && can_bypass_X && 
                        (rs2_addr_Dhl == rf_waddr_Xhl) && (rf_waddr_Xhl != 5'd0);

  wire rs2_M_byp_Dhl = rs2_en_Dhl && inst_val_Dhl && can_bypass_M &&
                       (rs2_addr_Dhl == rf_waddr_Mhl) && (rf_waddr_Mhl != 5'd0);
  wire rs2_X2_byp_Dhl = inst_val_Dhl && inst_val_X2hl && rs2_en_Dhl &&
                      rf_wen_X2hl && !is_md_X2hl && (rs2_addr_Dhl == rf_waddr_X2hl) &&
                      (rf_waddr_X2hl != 5'd0);
  wire rs2_X3_byp_Dhl = inst_val_Dhl && inst_val_X3hl && rs2_en_Dhl && rf_wen_X3hl && (rs2_addr_Dhl == rf_waddr_X3hl) &&
                      (rf_waddr_X3hl != 5'd0);
  wire rs2_W_byp_Dhl = inst_val_Dhl && inst_val_Whl && rs2_en_Dhl && rf_wen_Whl && (rs2_addr_Dhl == rf_waddr_Whl) &&
                      (rf_waddr_Whl != 5'd0);

  // Bypass order
  assign rs1_byp_sel_Dhl = rs1_X_byp_Dhl  ? 3'd1 : rs1_M_byp_Dhl  ? 3'd2 : rs1_X2_byp_Dhl ? 3'd3 :rs1_X3_byp_Dhl ? 3'd4 :
                               rs1_W_byp_Dhl  ? 3'd5 : 3'd0;  // rf

  assign rs2_byp_sel_Dhl = rs2_X_byp_Dhl  ? 3'd1 : rs2_M_byp_Dhl  ? 3'd2 : rs2_X2_byp_Dhl ? 3'd3 : rs2_X3_byp_Dhl ? 3'd4 :
                          rs2_W_byp_Dhl  ? 3'd5 : 3'd0;  //RF
  ...
  wire bubble_next_Dhl = ( !bubble_sel_Dhl ) ? bubble_Dhl
                       : ( bubble_sel_Dhl )  ? 1'b1
                       :                       1'bx;
  // Stall signal with load and mul and normal     
  ...
  // branch signal
  wire any_br_taken_Xhl = ( bne_taken_Xhl || blt_taken_Xhl || beq_taken_Xhl || bge_taken_Xhl || bltu_taken_Xhl || bgeu_taken_Xhl );
  wire brj_taken_Xhl = ( inst_val_Xhl && any_br_taken_Xhl );
  // Dummy Squash Signal

  wire squash_Xhl = 1'b0;

  // X stage don't stall for mul/div
  // Stall in X if muldiv reponse is not valid and there was a valid request
  //wire stall_md_Xhl = ( muldivreq_val_Xhl && inst_val_Xhl && !muldivresp_val );
